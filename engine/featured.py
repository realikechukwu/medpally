...
def score_candidate(candidate: FeaturedCandidate) -> ScoredCandidate:
    ...
    return ScoredCandidate(
        candidate,
        design + prestige + engagement,
        {"design": design, "prestige": prestige, "engagement": engagement},
    )


def rank_featured(
    candidates: list[FeaturedCandidate], *, limit: int = 10, min_score: float = MIN_SCORE
    ...
    return sorted(
        ...
        key=lambda item: (
            -item.score,
            -item.candidate.feed_date.toordinal(),
