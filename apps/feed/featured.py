...
def gather_candidates(specialty: Specialty, week_start: date) -> list[FeaturedCandidate]:
    ...
    papers = list(Paper.objects.filter(
        id__in=paper_ids, feed_date__gte=start, feed_date__lt=week_start,
        is_visible=True, summary_status=Paper.SummaryStatus.OK,
    ).exclude(id__in=excluded).select_related("journal", "summary"))
    ids = [paper.id for paper in papers]
    likes = {row["paper_id"]: row["n"] for row in UserPaperState.objects.filter(paper_id__in=ids, liked_at__isnull=False).values("paper_id").annotate(n=Count("id"))}
    saves = {row["paper_id"]: row["n"] for row in UserPaperState.objects.filter(paper_id__in=ids, saved_at__isnull=False).values("paper_id").annotate(n=Count("id"))}
    return [FeaturedCandidate(
        paper_id=paper.id, study_type=paper.summary.study_type,
        journal_prestige=paper.journal.prestige_weight, likes=likes.get(paper.id, 0),
        saves=saves.get(paper.id, 0), feed_date=paper.feed_date,
    ) for paper in papers]
...
def select_featured_for_week(specialty: Specialty, week_start: date, *, dry_run: bool = False):
    ...
    with transaction.atomic():
        ...
        FeaturedPaper.objects.bulk_create([
            FeaturedPaper(specialty=specialty, paper_id=item.candidate.paper_id, week_start=week_start,
                          rank=index, score=item.score)
